...
def retrospective_resampling_generate(
    model, tokenizer,
    original_input_ids, image_tensor,
    image_sizes, args, add_open_ended_qa_prompt=False,
) -> tuple[str, list, bool]:
    """
    ...
    def prepare_prefix():
        ...
        hint_string = " " + OPEN_ENDED_QA_QUERY_REWRITING_PROMPT if "llama_3" in args.conv_mode else OPEN_ENDED_QA_QUERY_REWRITING_PROMPT
        if len(all_hallucination_phrases) > 0:
            ...
            negative_object_string = " " + negative_object_string + " " if "llama_3" in args.conv_mode else negative_object_string
    ...
    for global_attempt in range(max_total_attempts):
        ...
        if args.verbose:
            ...
            visualize_prefix[visualize_prefix == IMAGE_TOKEN_INDEX] = 0
            print("Prefix: ", tokenizer.decode(visualize_prefix[0], skip_special_tokens=True))
