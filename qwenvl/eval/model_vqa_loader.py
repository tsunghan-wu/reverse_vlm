...
from .inference_utils import retrospective_resampling_generate, OPEN_ENDED_QA_DEFAULT_ANSWER
...
def eval_model(args):
    ...
    with torch.inference_mode():
        for inputs, metadata in tqdm(data_loader, total=len(data_loader)):
            ...
            input_data = (model, processor.tokenizer, inputs, args)
            ...
            write_result(question_id, cur_prompt, image_id, decoded_text, generation_history, successful_correction, ans_file, model_path)
        ans_file.close()

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    # Model settings
    parser.add_argument("--model-path", type=str, default="tsunghanwu/reverse_qwen25_vl")
    # Data settings
