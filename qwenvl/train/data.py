...
def query_injection(source, prob):
    N_convs = len(source["conversations"])
    for i in range(0, N_convs, 2):
        q = source["conversations"][i]["value"]
        ans_type = source["conversations"][i + 1].get("A_type", None)
        hint = source["conversations"][i + 1].get("hint", None)
        if ans_type == "general_answer":
            if random.random() < prob and hint is not None:
                new_q = f"{q} (Hint: potential incorrect phrases -> {hint.lower()})"
                source["conversations"][i]["value"] = new_q
    return source
...
class SupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources:
            ...
            images=None
            videos=None
        # Do de-hallucination training
        if self.data_args.do_dehallucination_training:
        ...
        for _, j in enumerate(range(0, len(sources), 2)):
            ...
            if DEFAULT_IMAGE_TOKEN in user_input:
                ...
                if "Qwen2.5" in self.model_id or "qwen2.5" in self.model_id or "qwen25" in self.model_id:
            ...
            labels = process_dehallucination_label(labels, processor.tokenizer)
