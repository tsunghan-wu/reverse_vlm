...
class CHAIR(object):
    ...
    def compute_chair(self, cap_file, image_id_key, caption_key):
        ...
        for i in tqdm.trange(len(caps)):
            ...
            all_caps_data.append({
                'len_words': len(words),
                'num_hallucinated': hallucinated_word_count,
                'is_hallucinated': int(hallucinated_word_count > 0),
                'len_gt': len(gt_objects),
                'recall_count': len(recall_gt_objects),
                'cap_len': len(raw_words),
            })

        # Compute overall metrics
        def compute_metrics(sample):
            total_caps = len(sample)
            chair_s = sum(x['is_hallucinated'] for x in sample) / total_caps
            chair_i = sum(x['num_hallucinated'] for x in sample) / (sum(x['len_words'] for x in sample) + 1e-8)
            recall = sum(x['recall_count'] for x in sample) / (sum(x['len_gt'] for x in sample) + 1e-8)
            avg_len = 0.01 * sum(x['cap_len'] for x in sample) / total_caps
            return chair_s, chair_i, recall, avg_len

        # Original full dataset results
        orig_chair_s, orig_chair_i, orig_recall, orig_avg_len = compute_metrics(all_caps_data)

        # Bootstrapping
        bootstrap_rounds = 100
        rng = np.random.default_rng(42)
        all_indices = list(range(len(all_caps_data)))
        boot_metrics = []

        for _ in tqdm.trange(bootstrap_rounds, desc="Bootstrapping"):
            sampled = [all_caps_data[i] for i in rng.choice(all_indices, size=len(all_caps_data), replace=True)]
            boot_metrics.append(compute_metrics(sampled))

        boot_metrics = np.array(boot_metrics)
        metric_names = ['CHAIRs', 'CHAIRi', 'Recall', 'Len']
        ...
        bootstrap_stats = {
            name: {
                'mean': float(np.mean(boot_metrics[:, i])),
                'std': float(np.std(boot_metrics[:, i])),
                '95ci': [float(np.percentile(boot_metrics[:, i], 2.5)),
                        float(np.percentile(boot_metrics[:, i], 97.5))]
            }
...
def print_metrics(overall_metrics, bootstrap_stats):
    print("=" * 50)
    for metric in ['CHAIRs', 'CHAIRi', 'Recall', 'Len']:
        val = overall_metrics.get(metric, float('nan')) * 100
        ci_low, ci_high = [x * 100 for x in bootstrap_stats.get(metric, {}).get('95ci', [None, None])]
        
        print(f"{metric}: {val:.2f}, 95% CI: {ci_low:.2f}–{ci_high:.2f}")
    print("=" * 50)
...
if __name__ == '__main__':
    ...
    print_metrics(cap_dict['overall_metrics'], cap_dict['bootstrap_stats'])
